    ...
    <div className="h-screen bg-[#0A0A0A] text-white flex items-center justify-center">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-[#666666] mt-1">Enter admin key to continue</p>
        </div>
        <form onSubmit={handleSubmit} className="space-y-4">
          <input
            ...
            autoFocus
            className="w-full px-4 py-3 bg-[#1A1A1A] border border-[#222222] rounded-xl text-sm text-white placeholder:text-[#666666] focus:outline-none focus:ring-2 focus:ring-red-500/50 focus:border-transparent"
          />
          {error && (
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold">Super Admin</h1>
              <p className="text-xs text-[#666666]">Platform-wide tenant management</p>
            </div>
          </div>
          <button
...
function StatCards({ tenants }: { tenants: TenantSummary[] }) {
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
      {stats.map(s => (
        <div key={s.label} className="bg-[#1A1A1A] border border-[#222222] rounded-xl p-5">
          <div className="flex items-center gap-3 mb-2">
            <Icon icon={s.icon} className={`w-5 h-5 ${s.color}`} />
            <span className="text-xs text-[#666666] uppercase tracking-wider">{s.label}</span>
          </div>
    ...
    <>
      ...
      {loading ? (
        <div className="flex items-center justify-center py-20">
          <Icon icon="svg-spinners:ring-resize" className="w-8 h-8 text-[#666666]" />
        </div>
      ) : (
              ...
              <thead>
                <tr className="border-b border-[#222222]">
                  {['Name', 'Slug', 'Plan', 'Status', 'Users', 'Projects', 'Created', 'Actions'].map(h => (
                    <th key={h} className="text-left px-5 py-3 text-[10px] font-medium text-[#666666] uppercase tracking-wider">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {tenants.map(t => (
                  <tr key={t.id} className="border-b border-[#222222]/50 hover:bg-[#0F1829] transition-colors">
                    ...
                    <td className="px-5 py-3 text-xs text-[#888888] font-mono">{t.slug}</td>
                    <td className="px-5 py-3">
                    ...
                    <td className="px-5 py-3">
                      <button
                        ...
                        className={`relative w-9 h-5 rounded-full transition-colors ${t.is_active ? 'bg-green-500' : 'bg-[#222222]'}`}
                      >
                    ...
                    <td className="px-5 py-3 text-sm text-[#888888] font-mono">{t.user_count}</td>
                    <td className="px-5 py-3 text-sm text-[#888888] font-mono">{t.project_count}</td>
                ...
                {tenants.length === 0 && !loading && (
                  <tr>
                    <td colSpan={8} className="px-5 py-12 text-center text-sm text-[#666666]">
...
function TenantDetailView({
  ...
  if (error || !detail) {
    return (
      <div>
        <button onClick={onBack} className="text-xs text-[#666666] hover:text-white mb-4 flex items-center gap-1">
  ...
  return (
    <div>
      <button onClick={onBack} className="text-xs text-[#666666] hover:text-white mb-6 flex items-center gap-1">
      ...
      <div className="space-y-6">
        ...
        <div className="bg-[#1A1A1A] border border-[#222222] rounded-xl p-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
            <div>
              <p className="text-[10px] text-[#666666] uppercase tracking-wider mb-1">Name</p>
              <p className="text-white font-medium">{t.name}</p>
            </div>
            <div>
              <p className="text-[10px] text-[#666666] uppercase tracking-wider mb-1">Slug</p>
              <p className="text-[#888888] font-mono">{t.slug}</p>
            </div>
            <div>
              <p className="text-[10px] text-[#666666] uppercase tracking-wider mb-1">Plan</p>
              <p className="text-[#888888]">{t.plan}</p>
            </div>
            <div>
              <p className="text-[10px] text-[#666666] uppercase tracking-wider mb-1">Status</p>
              <span className={`inline-flex px-2 py-0.5 rounded text-[10px] font-semibold ${
            ...
            <div>
              <p className="text-[10px] text-[#666666] uppercase tracking-wider mb-1">Colors</p>
              <div className="flex items-center gap-2">
                ...
                <span className="text-[#666666] text-xs font-mono">{t.primary_color} / {t.secondary_color}</span>
              </div>
            </div>
            <div>
              <p className="text-[10px] text-[#666666] uppercase tracking-wider mb-1">Storage</p>
              <p className="text-[#888888]">{formatBytes(detail.storage_bytes)}</p>
            </div>
            <div>
              <p className="text-[10px] text-[#666666] uppercase tracking-wider mb-1">Created</p>
              <p className="text-[#888888]">{new Date(t.created_at).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}</p>
            </div>
            {t.logo_url && (
              <div>
                <p className="text-[10px] text-[#666666] uppercase tracking-wider mb-1">Logo</p>
        ...
        <div className="bg-[#1A1A1A] border border-[#222222] rounded-xl overflow-hidden">
          ...
          {detail.users.length === 0 ? (
            <div className="px-6 py-8 text-center text-sm text-[#666666]">No users</div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead>
                ...
                <tbody>
                  {detail.users.map(u => (
                    <tr key={u.user_id} className="border-b border-[#222222]/50">
                      <td className="px-5 py-3 text-sm text-white">{u.name}</td>
                      <td className="px-5 py-3 text-sm text-[#888888]">{u.email}</td>
                      <td className="px-5 py-3">
        ...
        <div className="bg-[#1A1A1A] border border-[#222222] rounded-xl p-6">
          ...
          {detail.projects.length === 0 ? (
            <p className="text-sm text-[#666666]">No projects</p>
          ) : (
            <div className="flex flex-wrap gap-2">
              {detail.projects.map(p => (
                <span key={p.id} className="inline-flex px-3 py-1 rounded-lg text-xs font-medium bg-[#0F1829] border border-[#222222] text-[#888888]">
                  {p.name}
                </span>
              ))}
            </div>
          ...
          <div className="flex items-end gap-3">
            <div className="flex-1">
              ...
              <input
                ...
                placeholder={t.slug}
                className="w-full px-3 py-2 bg-[#0F1829] border border-red-500/20 rounded-lg text-sm text-white font-mono placeholder:text-[#666666] focus:outline-none focus:ring-2 focus:ring-red-500/50"
              />
            </div>
            <button
    ...
    <div>
      ...
      <div className="max-w-lg">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            <label className="block text-xs text-[#666666] mb-1.5">Company Name *</label>
            <input
              type="text"
              value={name}
          ...
          <div>
            <label className="block text-xs text-[#666666] mb-1.5">Slug *</label>
            <input
              type="text"
              value={slug}
              ...
              required
              className="w-full px-4 py-3 bg-[#1A1A1A] border border-[#222222] rounded-xl text-sm text-white font-mono placeholder:text-[#666666] focus:outline-none focus:ring-2 focus:ring-red-500/50 focus:border-transparent"
            />
            <p className="text-[10px] text-[#666666] mt-1">URL-safe identifier. Must be unique.</p>
          </div>

          <div>
            <label className="block text-xs text-[#666666] mb-1.5">Plan</label>
            <select
              value={plan}
              onChange={e => setPlan(e.target.value)}
              className="w-full px-4 py-3 bg-[#1A1A1A] border border-[#222222] rounded-xl text-sm text-white focus:outline-none focus:ring-2 focus:ring-red-500/50 focus:border-transparent"
            >
          ...
          <div>
            <label className="block text-xs text-[#666666] mb-1.5">Owner Email (optional)</label>
            <input
              type="email"
              value={ownerEmail}
              ...
              className="w-full px-4 py-3 bg-[#1A1A1A] border border-[#222222] rounded-xl text-sm text-white placeholder:text-[#666666] focus:outline-none focus:ring-2 focus:ring-red-500/50 focus:border-transparent"
            />
            <p className="text-[10px] text-[#666666] mt-1">If provided, an invite email will be sent after creation.</p>
          </div>
