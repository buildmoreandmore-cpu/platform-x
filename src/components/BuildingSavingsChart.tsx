            ...
            <div key={ph} className="flex items-center gap-1.5">
              <div
                className={`w-2.5 h-2.5 rounded-full ${
                  i < phaseIndex ? 'bg-primary' : i === phaseIndex ? 'bg-primary ring-2 ring-primary/30' : 'bg-gray-200'
                }`}
                title={ph}
              />
              {i < PHASES.length - 1 && <div className={`w-4 h-px ${i < phaseIndex ? 'bg-primary' : 'bg-gray-200'}`} />}
            </div>
          ))}
        </div>
    ...
    <div className="space-y-4">
      ...
      <div
        className={`rounded-lg px-4 py-2.5 text-sm font-medium flex items-center gap-2 ${
          aboveCount === 0
            ? 'bg-primary/10 text-[#228B17] border border-primary/30'
            : belowCount === 0
                  ...
                  <div className="w-full flex items-end justify-center gap-1.5 h-full">
                    <div className="w-2/5 bg-[#D4D4D4] rounded-t relative group" style={{ height: `${baseH}%` }}>
                      <div className="opacity-0 group-hover:opacity-100 absolute bottom-full mb-1 left-1/2 -translate-x-1/2 bg-gray-800 text-white text-[10px] py-1.5 px-2.5 rounded whitespace-nowrap z-10 pointer-events-none">
                        ...
                        <p className={saving ? 'text-secondary' : 'text-red-300'}>
        ...
        <div className="bg-[#F5F7FB] rounded-xl p-5 border border-gray-100">
          ...
          <div className="space-y-2">
            ...
            <div className="border-t border-gray-200 pt-2 flex justify-between text-xs">
              ...
              <span className={`font-semibold ${totalDiff >= 0 ? 'text-secondary' : 'text-red-600'}`}>
            ...
            <div className="flex justify-between text-xs">
              ...
              <span className={`font-semibold ${totalCO2 >= 0 ? 'text-secondary' : 'text-red-600'}`}>
          ...
          return (
            <div key={b.id} className="bg-[#F5F7FB] rounded-xl p-5 border border-gray-100">
              ...
              <div className="space-y-2">
                ...
                <div className="border-t border-gray-200 pt-2 flex justify-between text-xs">
                  <span className="text-gray-500">Difference</span>
                  <span className={`font-semibold ${saving ? 'text-secondary' : 'text-red-600'}`}>
