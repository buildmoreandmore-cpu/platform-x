...
export function ClientAccess({ projectId }: ClientAccessProps) {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      default:
        return 'bg-[#222222] text-[#888888] border-[#2A3A5C]';
    }
  };
      ...
      <div className="bg-[#1A1A1A] border border-[#222222] rounded-lg overflow-hidden">
        ...
        {invites.length === 0 ? (
          <div className="px-6 py-8 text-center">
            <Icon icon="solar:users-group-two-rounded-bold-duotone" className="w-12 h-12 text-[#666666] mx-auto mb-3" />
            <p className="text-sm text-[#888888]">No clients have been invited yet</p>
            <p className="text-xs text-[#666666] mt-1">Invite clients to give them access to the project portal</p>
          </div>
        ) : (
              ...
              <tbody className="divide-y divide-[#222222]">
                {invites.map((invite) => (
                  <tr key={invite.id} className="hover:bg-[#1A2544] transition-colors">
                    <td className="px-6 py-4">
                      <div className="flex items-center">
                        ...
                        <div>
                          ...
                          <div className="text-xs text-[#888888]">{invite.client_email}</div>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4">
                      <div className="text-sm text-[#888888]">{formatDate(invite.created_at)}</div>
                      <div className="text-xs text-[#666666]">by {invite.invited_by}</div>
                    </td>
                    <td className="px-6 py-4">
                      {invite.accepted_at ? (
                        <div className="text-sm text-[#888888]">{formatDate(invite.accepted_at)}</div>
                      ) : (
                        <div className="text-sm text-[#666666]">—</div>
                      )}
                    </td>
                    <td className="px-6 py-4">
...
function InviteClientModal({ onClose, onInvite, loading }: InviteClientModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
      <div className="bg-[#1A1A1A] border border-[#222222] rounded-xl max-w-md w-full">
        <div className="px-6 py-4 border-b border-[#222222]">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full px-3 py-2 bg-[#0F1829] border border-[#222222] rounded-lg text-white placeholder-[#666666] focus:outline-none focus:border-primary/60 focus:ring-1 focus:ring-primary/30"
          ...
          <div className="flex items-center justify-end gap-3 pt-4">
            <button
              type="button"
              onClick={onClose}
              className="px-4 py-2 text-sm font-medium text-[#888888] hover:text-white transition-colors"
            >
              Cancel
            </button>
            <button
              type="submit"
              disabled={loading}
              className="inline-flex items-center gap-2 px-4 py-2 bg-primary text-[#0A0A0A] text-sm font-semibold rounded-lg hover:bg-[#00ff88] disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
            >
              {loading ? (
                <>
